...
LOCKED (design) — implemented 2026-09-03, steps 0–3 (see the implementation notes at the end of this section). The trigger below is measured on HEAD `ace4d55f`; the implementation order and gates are part of this amendment. Supersedes exactly one clause of Amendment D (its shared-mutable pairing, `\`Arc<struct{ lock: Locker; …data }>\``); preserves the rest of D and all of E.
...
\### Implementation notes (step 2, 2026-09-03)

`Locked<T>` is a compiler-known `Ref` carrying `HasAsgn | IsAtomic | IsLocked`, whose pointee is a generated struct `msLockedCell_<hash>` laid out `{ lockNext: int32, lockServing: int32, value: T }`. The two leading int32s ARE the ticket lock and sit at offset 0, so the runtime casts the handle straight to `msTicketLock*` and never learns T; the payload is reached only through the backend's `msLockedLoad` / `msLockedStore` intercepts, which take the field offset off that same generated struct. Because the handle is an atomic `Ref`, \*\*no new lifetime machinery was introduced\*\*: atomic incref/decref in classify, the SHARE boundary incref at the msgSetter, the Arc write-block and destructor lifting all apply to it unchanged, and the cell is ONE `msAllocArc` allocation whose zero-fill is already a valid unlocked ticket lock (no init call, as Amendment E specified).

Runtime consolidation: with the double-slot Locker surface retired, the ticket lock has exactly one consumer, so `promise/lockerLayout.h` + `promise/locker.h` + `promise/locked.h` collapse into a single `promise/locked.h` (atomics → address-keyed park → `msTicketLock` → cell acquire/release). The `msLocker` struct and its eleven `--gc=manual` stubs are gone; manual mode keeps two no-op `msLocked*` stubs, having no threads to exclude.

Two deliberate narrowings of the rule text — landing scope, not model changes:

\- \*\*Payload is deep-POD, not merely sendable.\*\* `isLockedPayloadSendable` takes primitives and struct / tuple / generic-instance payloads recursively (a `GenericParam` passes through and is re-checked at instantiation) and rejects strings, arrays, any `Ref` and functions. The store is a raw field assignment with no destroy-of-previous, so an owning payload would leak one value per update. Widening it is gated on giving the store intercept destroy-on-store; the egress rule is unaffected either way, since what leaves `lockedRead` is a copy.

\- \*\*The surface spells the verbs `lockedUpdate` / `lockedRead`\*\*, not the bare `update` / `read` of the rule text. `std/core/promise` is prelude, auto-loaded into every program; two names that common would be a permanent squat on user identifiers.

Mechanics worth knowing before the next change here: a generic alias (`type Locked<T> = Arc<…>`) does not resolve, so `Locked` must be a compiler-known name in the parser's built-in generic table, the resolve pass and `checkNewExpr`; and an extern carrying a `from` native name is emitted through an early branch of `emitCallExpr`, so the load/store intercepts only fire after `nativeLower` detaches those two symbols.

Gates, all on the self-hosted candidate: inline suite 3533 / 176 files under drc and orc; corpus full 656 pass · 0 fail · 0 xfail · 0 xpass — corpus 405's three `@xfail` lanes flipped to pass, the acceptance cue this amendment named; the errorcheck guards for S1/E61, S3/E62 and the three mutable-capture shapes still fail their builds with the right tags; and the candidate builds the next generation, which then compiles the `Locked` programs (bootstrap stable across the new prelude type). The SAN lane still needs a Linux host.

\### Implementation notes (step 3, 2026-09-03)

The three verb pins this amendment's gate named, all shipped:

\- \*\*The SEND-UAF shape proven red.\*\* `guard/paralockActorVerbs.ms` (GUARD-CHECK-FAIL) pins the five compile-time rules: SEND-class plain ref, discarded-CALL borrow, Arc pointee not deep-immutable, write through an Arc handle, Locked payload owning memory. Proven red by rebuilding the compiler with exactly those five checks neutered — the pre-amendment binary cannot serve as the red baseline directly, because it cannot read today's prelude (`Locked`); cases 1–4 then build CLEAN on the neutered compiler and fail with the right tags on the real one. Case 5 is weaker by nature: the checker also goes silent, but the build then dies at link (the cell's destroy hook is undefined for an owning payload), so its diagnostic turns an unexplained link failure into an explained one.

\- \*\*Arc→actor CALL and SEND.\*\* Corpus `415-arcActorBoundary`: one actor with a void `absorb(Arc<Payload>)` (SEND — the sender never parks) and an awaited `weigh(Arc<Payload>)` (CALL — mechanically a BORROW); the sender drops its last reference at once and a fresh Arc is allocated every round, so a freed cell is handed straight to the next allocation — the churn shape of the trigger-1 UAF. send=14000 (2000×7) proves the SEND payload outlived the sender's scope exit; call=10000 (2000×5) proves the CALL leg; flat RSS (~5.8 MB against a 40 bound) proves the boundary incref is released, not leaked, on both paths.

\- \*\*A Locked counter under spawn contention.\*\* Already shipped as the step-2 rewrite of corpus 405.
