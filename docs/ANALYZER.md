...
- Why `inject.ms` still has cases for `ForStmt`, `ForOfStmt`, `MatchStmt` and `SwitchStmt` while `PIPELINE.md` says Phase 3 lowers all of them — not traced.
- `cfg.ms` (last-read over a control-flow graph), `alias.ms` (path aliasing) and `cursors.ms` (borrow inference with live ranges and mutation tracking) are named, not described: their internals were not read in this pass.
