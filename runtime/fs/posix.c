...
msString msFsReadFile(msString path) {
	...
	if (fseeko(f, 0, SEEK_END) != 0) { _msFsLastErrno = errno; fclose(f); return MS_EMPTY_STRING; }
	off_t sz = ftello(f);
	if (sz < 0 || fseeko(f, 0, SEEK_SET) != 0) { _msFsLastErrno = errno; fclose(f); return MS_EMPTY_STRING; }
