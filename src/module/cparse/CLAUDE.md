...
**Status**: core pipeline complete (tokenize → preprocess → parse → eval).
Wired into the compiler via `src/module/cimport/emit.ms`.

**Location**: `src/module/cparse/` — pure chibicc port, no dependencies on
compiler internals. Only depends on `std/` (stdlib).
...
├── obj.ms            Obj (top-level declarations)
├── eval.ms           integer constant expression evaluator
├── unicode.ms        UTF-8 + UCN helpers
├── error.ms          diagnostic collection (no exit)
└── test.ms           unit tests per phase

src/module/cparse.ms  C→MS emitter (consumer: imports src/cparse/, emits extern decls)
```

**Rules**:

- **Zero dependencies on the compiler.** This module imports only from
  `std/` (stdlib). It does NOT import from `src/ast/`, `src/checker/`,
  `src/parser/`, `src/compiler/`, or any compiler internals.
...
- **No `exit()`.** Chibicc calls `error()` → `exit(1)` on any parse
  error. We collect diagnostics and continue. A single bad declaration
